...
void main()
{
	...
	perc(&math,&science,&english,&marathi,&hindi);	// in pointer we use '&' symbol for passing address.
}
void perc(float* m,float* sc,float* en,float* ma,float* hin)
{

	float total,percentage;
	total= *m+*sc+*en+*ma+*hin;  // all pointers shows values store at there addreses,
